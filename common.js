 ...
 * OmniBox 采集站爬虫公共模块
 *
 * 提供可复用的核心功能，各资源站脚本只需配置元数据和 API 地址
 */
...
let SPIDER_NAME = "";
let SPIDER_AUTHOR = "";
let SPIDER_VERSION = "1.0.0";
let SPIDER_DESCRIPTION = "刮削：支持，弹幕：支持，嗅探：支持";
let SPIDER_DOWNLOAD_URL = "";

/**
 * 配置爬虫参数
 * @param {Object} config - 配置对象
 */
function configureSpider(config) {
  SITE_API = config.SITE_API || process.env.SITE_API || "";
  DANMU_API = config.DANMU_API || process.env.DANMU_API || "";
  SPIDER_NAME = config.name || "";
  SPIDER_AUTHOR = config.author || "";
  SPIDER_VERSION = config.version || "1.0.0";
  SPIDER_DESCRIPTION = config.description || "刮削：支持，弹幕：支持，嗅探：支持";
  SPIDER_DOWNLOAD_URL = config.downloadURL || "";
}
